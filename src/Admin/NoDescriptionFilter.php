...
class NoDescriptionFilter {
	...
	public function init(): void {
		add_action( 'restrict_manage_posts', array( $this, 'add_empty_description_filter' ), 10, 2 );
		add_action( 'pre_get_posts', array( $this, 'filter_products_by_empty_description' ) );
	}
	...
	public function filter_products_by_empty_description( \WP_Query $query ): void {
		...
		if ( isset( $_GET['empty_description'] ) ) {
			...
			if ( $filter_value === '1' ) {
				add_filter( 'posts_where', array( $this, 'filter_where_empty_content' ) );
	...
	public function filter_where_empty_content( string $where ): string {
		...
		remove_filter( 'posts_where', array( $this, 'filter_where_empty_content' ) );
