...
class Logger {
	 ...
	 * @return void
	 */
	private static function write_log( string $level, string $message, array $context = array() ): void {
		if ( ! self::should_log() ) {
			return;
		}
	 ...
	 * @return void
	 */
	public static function log_api_request( string $endpoint, array $payload = array(), string $method = 'POST' ): void {
		self::debug(
			'API Request',
			array(
				'endpoint'     => $endpoint,
	 ...
	 * @return void
	 */
	public static function log_product_processing( int $product_id, string $action, string $status, array $additional_data = array() ): void {
		self::info(
			'Product Processing',
			array_merge(
				array(
